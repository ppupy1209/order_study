...
public interface UserCouponRepository extends JpaRepository<UserCoupon, Long> {
    ...
    @Query("""
            update UserCoupon uc
               set uc.status = com.example.orderstudy.coupon.UserCouponStatus.USED,
                   uc.usedAt = :now
             where uc.id = :userCouponId
               and uc.user.id = :userId
               and uc.status = com.example.orderstudy.coupon.UserCouponStatus.ISSUED
    ...
    @Query("""
            update UserCoupon uc
               set uc.status = com.example.orderstudy.coupon.UserCouponStatus.ISSUED,
                   uc.usedAt = null
             where uc.id = :userCouponId
               and uc.status = com.example.orderstudy.coupon.UserCouponStatus.USED
            """)
